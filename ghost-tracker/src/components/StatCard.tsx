...
export function StatCard({ label, value, icon: Icon, accent = 'default', suffix }: Props) {
  return (
    <div className="bg-surface border border-border rounded-xl p-5 flex items-center gap-4 hover:border-muted transition-colors">
      <div className={clsx('p-2.5 rounded-lg', accentMap[accent])}>
        <Icon size={18} />
      </div>
      <div>
        <div className="flex items-baseline gap-1">
          <span className="text-2xl font-display font-semibold text-text-primary">{value}</span>
