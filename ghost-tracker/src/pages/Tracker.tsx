  ...
  return (
    <div className="p-4 sm:p-8 max-w-6xl mx-auto animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="font-display font-semibold text-2xl sm:text-3xl text-text-primary mb-0.5">
            Applications
          </h1>
          <p className="font-mono text-xs text-text-muted">
            {applications.length} total ·{" "}
            {applications.filter((a) => a.status === "ghosted").length} ghosted
          </p>
        </div>
        {/* <button
          onClick={() => {
        ...
        {STATUS_FILTERS.map(({ value, label }) => {
          ...
          return (
            ...
            >
              {label}
              <span
                className={`px-1.5 py-0.5 rounded-full text-[10px] ${isActive ? "bg-accent/20 text-accent" : "bg-border text-text-muted"}`}
              >
                {count}
              </span>
            </button>
      ...
      <div className="hidden sm:block bg-surface border border-border rounded-2xl overflow-hidden">
        <div className="grid grid-cols-[2fr_1fr_1fr_1.5fr_80px] gap-4 px-6 py-3 border-b border-border">
          {["Company / Role", "Status", "Applied", "Ghost Meter", ""].map(
            (h) => (
        ...
        {!hasLoaded ? (
          <div className="p-6 space-y-4">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="grid grid-cols-[2fr_1fr_1fr_1.5fr_80px] gap-4 animate-pulse"
              >
          ...
          <div className="divide-y divide-border">
            {filtered.map((app) => (
              <div
                key={app.id}
                className="grid grid-cols-[2fr_1fr_1fr_1.5fr_80px] gap-4 items-center px-6 py-4 hover:bg-white/[0.02] transition-colors"
              >
